...
export default function AdminSearch() {
  ...
  return (
    <div ref={boxRef} className="relative px-3 pb-2">
      ...
      {open && results ? (
        <div className="absolute inset-x-3 top-full z-50 mt-1 max-h-80 overflow-y-auto thin-scroll rounded-xl border border-slate-700 bg-[#0a0f1d] p-2 shadow-2xl">
          {empty ? (
                ...
                <>
                  ...
                  {results.clients.map((c) => (
                    <button
                      ...
                      className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-right text-sm text-slate-200 hover:bg-slate-800"
                    >
                      <Icon name="users" className="h-3.5 w-3.5 text-slate-500" />
                      {c.name}
                      {!c.active ? <span className="text-[10px] text-red-400">מושבת</span> : null}
                    </button>
                  ))}
                </>
              ) : null}
