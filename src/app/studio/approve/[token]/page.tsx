...
export default function ApprovePage({ params }: { params: { token: string } }) {
  ...
  if (!loaded) {
    return <div className="flex min-h-screen items-center justify-center bg-slate-950 text-slate-400">טוען…</div>;
  }
  if (err || !task) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-950 px-6 text-center text-slate-300">
        <div>
          <h1 className="mb-2 text-2xl font-black tracking-tight text-slate-100">APOLLO<span className="text-cyan-400">ADV</span></h1>
    ...
    <div className="min-h-screen bg-slate-950 px-4 py-8 text-slate-200">
      <div className="mx-auto max-w-2xl">
        <header className="mb-6 text-center">
          <h1 className="text-2xl font-black tracking-tight text-slate-100">APOLLO<span className="text-cyan-400">ADV</span></h1>
        ...
        <div className="mb-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
          <div className="mb-1 flex items-center gap-2">
            ...
            <span className="rounded-full bg-indigo-500/15 px-2 py-0.5 text-xs text-indigo-300">{BRIEF_LABELS[task.briefType] || task.briefType}</span>
            {task.round > 1 ? <span className="rounded-full bg-orange-500/15 px-2 py-0.5 text-xs text-orange-300">סבב {task.round}</span> : null}
          </div>
          {approved ? (
            <p className="mt-2 rounded-xl border border-emerald-700/40 bg-emerald-600/10 px-3 py-2 text-sm text-emerald-300">
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {task.assets.map((a) => {
            ...
            return (
              <div key={a.id} className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/40">
                <a href={`/api/studio/approve/${token}/asset/${a.id}`} target="_blank" rel="noopener noreferrer" className="block">
                  {isImage(a) ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={`/api/studio/approve/${token}/asset/${a.id}`} alt={a.fileName || "asset"} className="h-48 w-full bg-slate-800 object-cover" />
                  ) : (
                ...
                <div className="flex items-center justify-between px-3 py-2 text-xs">
                  ...
                  <button onClick={() => setOpenAsset(open ? null : a.id)} className="shrink-0 text-cyan-300 hover:text-cyan-200">
                ...
                {open ? (
                  <div className="border-t border-slate-800 p-3">
                    <div className="mb-2 flex flex-col gap-1.5">
                      {comments.map((m) => (
                        <div key={m.id} className={`rounded-xl px-2.5 py-1.5 text-xs ${m.authorSide === "client" ? "bg-cyan-600/15 text-slate-100" : "bg-slate-800/60 text-slate-200"}`}>
                    ...
                    <div className="flex items-end gap-2">
                      ...
                      />
                      <button disabled={busy} onClick={() => sendMsg(assetComment[a.id] ?? "", a.id)} className="rounded-lg bg-cyan-500/20 px-3 py-1.5 text-xs text-cyan-200 hover:bg-cyan-500/30 disabled:opacity-50">שליחה</button>
                    </div>
                  </div>
                ) : null}
              </div>
        ...
        {waiting ? (
          <div className="mb-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
            <textarea
              ...
              className="mb-3 w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-200"
            />
            <div className="flex gap-2">
              ...
              <button disabled={busy} onClick={() => decide("changes")} className="rounded-xl border border-slate-700 px-4 py-2.5 text-sm text-slate-200 hover:bg-slate-800 disabled:opacity-50">
                בקשת שינויים
              </button>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
          <p className="mb-2 text-sm font-bold text-slate-300">התכתבות עם הסטודיו</p>
          <div className="mb-3 flex max-h-72 flex-col gap-2 overflow-y-auto">
            {generalMsgs.map((m) => {
              const mine = m.authorSide === "client";
              return (
                <div key={m.id} className={`max-w-[85%] rounded-2xl px-3 py-2 ${mine ? "self-end bg-cyan-600/15" : "self-start bg-slate-800/60"}`}>
          ...
          <div className="flex items-end gap-2">
            ...
            />
            <button disabled={busy || !chat.trim()} onClick={() => sendMsg(chat, null)} className="rounded-xl bg-cyan-500/20 px-4 py-2.5 text-sm text-cyan-200 hover:bg-cyan-500/30 disabled:opacity-50">שליחה</button>
          </div>
        </div>
