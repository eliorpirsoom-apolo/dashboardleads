...
## 4ב) יומני Google של הצוות בלוח המשרד — ✅ פעיל (20.07.2026)

כל עובד משרד מחבר את היומן שלו בקליק: **לוח שנה → "+ חיבור היומן שלי"** →
בחירת חשבון → Advanced → Continue (אזהרת "אפליקציה לא מאומתת" — תקין, scope
רגיש ללא אימות Google) → אישור. מאותו רגע:

- הלוח המשותף מציג את **כל** היומנים של **כל** העובדים המחוברים (צבע פר-עובד,
  לחיצה על שם עובד בסרגל מסתירה/מציגה אותו).
- **דו-כיווני:** משימה/פגישה של צד משרד שנוצרת במערכת נכתבת אוטומטית ליומן
  Google של המטפל (או היוצר), מתעדכנת בעריכה ונמחקת במחיקה.
- ניתוק: "ניתוק היומן שלי" בלוח, או הסרת ההרשאה ב-myaccount.google.com/permissions.

דרישות (בוצעו): Google Calendar API מופעל בפרויקט apollo-crm-502908; אין צורך
ב-redirect URI נוסף (משתמש ב-callback הקיים של האינטגרציות). חובר ואומת
קצה-לקצה עם eliorbucris@gmail.com — כולל יצירה/מחיקה של אירוע מהמערכת.
